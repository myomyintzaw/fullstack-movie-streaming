...
const SaveMovie = () => {
    ...
    useEffect(() => {
        axios.get('/api/get-saved-movies').then((d)=>{
            setLoader(false);
            setData(d.data);
        });
    ...
    {!loader && (
      ...
      {data.map((d) =>(
        ...
        <div key={d.movie.id} className="col-6 col-sm-6 col-md-3 col-lg-3">
        <a href={`/movie/${d.movie.slug}`} className="text-decoration-none">
         <div
            className="movie-card-container position-relative d-flex justify-content-center align-items-center"
            style={{ backgroundImage:`url("${d.movie.image}")`,
            }}>

             {/* rating */}
             <div className="rating position-absolute rounded-circle d-flex justify-content-center align-items-center">
              <span className="text-white">{d.movie.rating_no}</span>
            </div>
             {/* play icon */}
